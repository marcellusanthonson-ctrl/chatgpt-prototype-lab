...
const executionRel = 'projects/lab/test-executions/PRODUCT-LEADERSHIP-ACTIVATION-AND-VALUE-DISCRIMINATION-TEST-EXECUTION-003';
...
const runner = path.join(workspace, '.codex-temp/runner-184/codex-x86_64-pc-windows-msvc-0.146.0.exe');
...
async function listInputFiles() {
  const designFiles = (await fsp.readdir(path.join(workspace, designRel))).sort().map(n => `${designRel}/${n}`);
  const packageFiles = (await fsp.readdir(path.join(workspace, packageRel))).sort().map(n => `${packageRel}/${n}`);
  const executionInputs = [
    ...
    'SCORER_PROMPT.md', 'CALIBRATION_PROMPT.md', 'CALIBRATION_CASES.json',
    'GENERATION_OUTPUT_SCHEMA.json', 'CALIBRATION_OUTPUT_SCHEMA.json', 'SCORING_OUTPUT_SCHEMA.json',
    'RUN_TEST003.mjs', 'RUN_PLAN.json', 'ARM_MAPPING.json'
  ].map(n => `${executionRel}/${n}`);
  return [
    ...designFiles, ...packageFiles, ...executionInputs,
    'projects/lab/authorizations/AUTHORIZATION_LAB_PRODUCT_LEADERSHIP_TEST003_REPOSITORY_ONLY_PREFLIGHT_RECONCILIATION_AND_CONDITIONAL_EXECUTION_182.json',
    'projects/lab/authorizations/AUTHORIZATION_LAB_PRODUCT_LEADERSHIP_TEST003_NETWORKED_PREAUTHORIZED_CODEX_EXECUTION_184.json',
    'projects/lab/briefs/CODEX_PRODUCT_LEADERSHIP_TEST003_NETWORKED_PREAUTHORIZED_EXECUTION_184_001.json'
  ];
}
...
async function selfTest() {
  ...
  for (const name of ['GENERATION_OUTPUT_SCHEMA.json', 'CALIBRATION_OUTPUT_SCHEMA.json', 'SCORING_OUTPUT_SCHEMA.json']) await readJson(`${executionRel}/${name}`);
  if (!fs.existsSync(runner)) throw new Error('Ephemeral runner missing');
...
async function verifyFinal() {
  ...
  if (inputFreeze.status !== 'FROZEN_BEFORE_GENERATION' || outputManifest.output_count !== 112 || scoringFreeze.score_count !== 112 || calibration.status !== 'PASS' || status.outputs_generated !== 112) throw new Error('FINAL_INVARIANT_FAILURE');
...
const actions = { 'self-test': selfTest, 'freeze-inputs': freezeInputs, 'verify-freeze': async () => { const f = await verifyFreeze(); console.log(JSON.stringify({ mode, result: 'PASS', inputs: f.input_count })); }, calibrate, generate, 'freeze-outputs': freezeOutputs, score, analyze, 'verify-final': verifyFinal };
